...
function sanitizeOutput(md) {
  ...
  try {
    ...
    // Remove explicit paired spans in one pass using a broad, non-greedy regex
    const broadPairs = [
    ...
    // As a fallback, iteratively remove any remaining segments between detected open/close triangle tokens
    while (true) {
      ...
      const endIdx = openIdx + 1 + closeMatch + 1; // include closing symbol
